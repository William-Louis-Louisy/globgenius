...
export function useShapeQuiz({
  ...
  const load = useCallback(async () => {
    ...
    try {
      const data: ShapeRandomResponse = await getRandomShapeQuestion(locale);
      setShape(data.question.data);
  ...
  const hints = useMemo(() => {
    const wrong = attempts - attemptsLeft;

    const showRegion = wrong >= 2 && !!answerDetails?.region;
    const showCapital = wrong >= 3 && !!answerDetails?.capital;
    const showFirstLetter = wrong >= 4 && !!answerLocalized[0];
    return {
  ...
  }, [attempts, attemptsLeft, answerDetails, answerLocalized]);
